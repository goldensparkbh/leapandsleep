...
function GuideCard({ post, index }: { post: Post; index: number }) {
          ...
          <div className="h-[38%] p-6 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-3 mb-3">
                <BookOpen className="w-4 h-4 text-[#B8B1F5]" />
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm text-[#6D727A]">{post.readingTime} min read</span>
...
export function PopularGuidesSection() {
  ...
  const { getPostsBySection } = useData();
  const guides = getPostsBySection('start-the-leap').slice(0, 3);
  ...
  return (
    <section
      ref={sectionRef}
      className="relative w-full bg-[#F6F7F9] py-20 lg:py-28 z-[80]"
    >
      <div className="w-full px-6 lg:px-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {guides.map((post, index) => (
            <GuideCard key={post.id} post={post} index={index} />
          ))}
        </div>
      </div>
    </section>
